...
.factory('SearchService', function($log, DB){
  ...
  function search_book(keyword, offset, limit){
    $log.debug( keyword, offset, limit );
    var sql = "SELECT cid, seg_id, chapter_name, law_name snippet(qa_fts) as highlights FROM book_fts WHERE book_fts match ? LIMIT ?, ?";
    return DB.queryForList(sql, [keyword, offset, limit]);
  }
  ...
  function buildIndex(success, error){
      ...
      var sql_build = "INSERT INTO qa_fts(id, question, published_at, real_seq, paper) SELECT id, question, published_at, real_seq, paper FROM question_answer";
      //创建书籍索引
      var sql_book_create = "CREATE VIRTUAL TABLE IF NOT EXISTS book_fts(law_name, cid, chapter_name, seg_id, seg_title, tokenize=icu zh_CN)";
